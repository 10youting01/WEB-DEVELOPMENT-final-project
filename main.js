...
; ~function (global) {
    ...
    function ext(base, obj) { //ext : js的應用程式框架
        for (var key in obj) {
            if (obj.hasOwnProperty(key)) { //檢查是否有key這一屬性
                base[key] = obj[key]
            }
        }
        return base;
    ...
    //載入圖片
    function loadImage(inMap, onload) { //inMap是名稱:圖片的array
        var outMap = {}; //function
        var keyArr = []; //array
        for (var key in inMap) {
            keyArr.push(key); //把圖片名稱加到keyArr
        }
        ...
        var doLoadImage = function (keyArr) {
            ...
            loading += len;
            for (var i = 0; i < len; ++i) { //讀圖片位置、新增圖片到outMap
                key = keyArr[i];
                var url = inMap[key];
                var img = new Image();
                outMap[key] = img;
    ...
    var HERO_WIDTH = 26;
    var ARROW_HEIGHT = 15, ARROW_WIDTH = 5; // 釘子尺寸

    var FLOOR_VELOCITY_BASE = -0.20; // 地板上升速度-變快
    var GRAVITY_ACC = 0.0018; // 重力加速度
    var SPRINGING_VELOCITY = -0.5; // 離開弹簧時的初速度
    var SPRING_TIME = 100; // 弹簧壓缩時間
    var FAKE_FLOOR_TIME = 200, FAKE_FLOOR_TIME2 = 500; // 虛踏板的停留時間, 虛踏板的转动時間
    var ROLLING_VELOCITY = 0.2; // 傳送带速度-加快
    var CONTROL_VELOCITY = 0.19; // 左右操作的速度-變慢
    ...
    function drawAll(context, time) {
        ...
        if (!isRunning) {
            ...
            if (isCooldownTime) {
                ...
                context.textAlign = 'center';
                context.fillText(judge() ? 'Jump Again' : 'Continue', 0, 10);
            }
            context.restore();
        }
    ...
    Game.launch = function ($wrapNode) {
        ...
        loadImage({ //loadImage function
            bg: "./MainPageImg/bg.png",
            hero: "./MainPageImg/led.png"
        }, init);
    }
